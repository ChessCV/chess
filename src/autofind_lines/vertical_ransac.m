...
% 	(returns them from left to right)
    ...
    % add y intercepts
	lines_rt = [lines; get_y_intercepts(lines, image_height)];
    
    % sort by y intercepts
	[Y, sort_permutation] = sort (lines_rt, 2, 'ascend');	
    
    % stores the permutation for sorting thetas
	sort_permutation = sort_permutation(3, :);	
